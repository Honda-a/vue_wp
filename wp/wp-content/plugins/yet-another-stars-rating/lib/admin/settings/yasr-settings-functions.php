...
function yasr_custom_text_callback( $option )
{
    ...
    <div id="yasr-doc-custom-text-div" class="yasr-help-box-settings">
        ...
        <?php 
    $string_custom_overall = sprintf( __( 'In the first field you can use %s pattern to show the overall rating.', 'yet-another-stars-rating' ), '<strong>%overall_rating%</strong>' );
    $string_custom_visitor = sprintf( __( 'In the Second and Third fields you can use %s pattern to show the 
        total count, and %s pattern to show the average', 'yet-another-stars-rating' ), '<strong>%total_count%</strong>', '<strong>%average%</strong>' );
...
function yasr_general_options_sanitize( $option )
{
    ...
    foreach ( $option as $key => $value ) {
        ...
        if ( isset( $option[$key] ) ) {
            ...
            if ( $key == 'text_before_overall' || $key == 'text_before_visitor_rating' || $key == 'text_after_visitor_rating' || $key == 'custom_text_must_sign_in' || $key == 'custom_text_user_voted' ) {
