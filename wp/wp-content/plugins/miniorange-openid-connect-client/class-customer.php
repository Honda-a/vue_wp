<?php
/** miniOrange enables user to log in through OpenIDConnect to apps such as Amazon Cognito, SalesForce etc.
...
* @package 		miniOrange OpenID Connect
...
class Customer {
	...
	function create_customer(){
		...
		$ch = curl_init( $url );
		$this->email 		= get_option('mo_openidconnect_admin_email');
		$this->phone 		= get_option('mo_openidconnect_admin_phone');
		$password 			= get_option('password');
		$firstName    		= get_option('mo_openidconnect_admin_fname');
		$lastName     		= get_option('mo_openidconnect_admin_lname');
		$company      		= get_option('mo_openidconnect_admin_company');
		
		$fields = array(
			'companyName' => $company,
			'areaOfInterest' => 'WP OpenIDConnect Client',
			'firstname'	=> $firstName,
			'lastname'	=> $lastName,
			'email'		=> $this->email,
			'phone'		=> $this->phone,
	...
	function add_openidconnect_application( $name, $app_name ) {
		
		$url = get_option('host_name') . '/moas/rest/application/addopenidconnect';
		$ch = curl_init( $url );
		
		$customerKey = get_option('mo_openidconnect_admin_customer_key');
		$scope = get_option('mo_openidconnect_' . $name . '_scope');
		$client_id = get_option('mo_openidconnect_' . $name . '_client_id');
		$client_secret = get_option('mo_openidconnect_' . $name . '_client_secret');
		if($scope != false) {
			$fields = array(
	...
	function submit_contact_us( $email, $phone, $query ) {
		global $current_user;
		wp_get_current_user();
		$query = '[WP OpenIDConnect Client] ' . $query;
		$fields = array(
	...
	function send_otp_token(){
			...
			$username = get_option('mo_openidconnect_admin_email');
	...
	function check_customer() {
		...
		function mo_openidconnect_send_email_alert($email,$phone,$message){
		...
		$authorizationHeader= "Authorization: " . $hashValue;
		$fromEmail 			= $email;
		$subject            = "Feedback: WordPress OpenID Connect Client Plugin";
		$site_url=site_url();

		global $user;
		$user         = wp_get_current_user();
		$query        = '[WP OpenID Connect Client] : ' . $message;
